...
def index(request):
    ...
    # statuses = api.GetUserTimeline(screen_name="fanzhang312", count=5)
    statuses = api.GetUserTimeline(screen_name="umnSUA", count=3, include_rts=False, include_entities=True, exclude_replies=True)

    status = jsonpickle.encode(statuses)
